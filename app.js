...
function addClient(){let no=next(db.clients,"no","S-"),nom=prompt("Nom:");if(!nom)return;let prenom=prompt("Prénom:")||"",nif=prompt("NIF/CIN:")||"",adr=prompt("Adresse:")||"",tel=formatPhone509(prompt("Tel:")||""),hands=Number(prompt("Nbre(s) de main(s):","1"))||1;db.clients.push({no,nom,prenom,nif,adr,tel,hands});logAction("Ajoute kliyan",no+" - "+nom+" "+prenom);save()}function editClient(i){auth(()=>{let c=db.clients[i];c.nom=prompt("Nom:",c.nom)||c.nom;c.prenom=prompt("Prénom:",c.prenom)||c.prenom;c.nif=prompt("NIF/CIN:",c.nif)??c.nif;c.adr=prompt("Adresse:",c.adr)??c.adr;c.tel=formatPhone509(prompt("Tel:",c.tel)??c.tel);c.hands=Number(prompt("Nbre(s) de main(s):",c.hands))||c.hands;save()})}function del(t,i){auth(()=>{if(confirm("Responsab: ou konfime efase sa a?")){db[t].splice(i,1);save()}})}function addPay(){
...
async function loadCloudCore(){
 ...
 db.clients=(cs||[]).map(c=>({no:c.client_no,nom:c.nom,prenom:c.prenom||"",nif:c.nif_cin||"",adr:c.address||"",tel:formatPhone509(c.phone||""),hands:Number(c.hands||1)}));
